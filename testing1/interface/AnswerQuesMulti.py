...
import requests
import random
from testing1.interface import InterfaceMultiPara
...
def runAnswer(phone):
    print("开始获取验证码")
    urlgetcodeRe = requests.post(urlgetcode, json=InterfaceMultiPara.login_get_code(phone))
    print("获取验证码接口返回")
    print(urlgetcodeRe.text)

    print("开始验证登陆")
    urlloginRe = requests.post(urllogin, json=InterfaceMultiPara.login(phone))
    print("登陆验证返回")
    ...
    urlmainpageRe = requests.post(urlmainpage, json=InterfaceMultiPara.win_mainpage(session))
    print("mainpage接口返回为")
    print(urlmainpageRe.text)

    for i in range(1, 4):
        print("测验开始接口开始调用,次数为：" + str(i))
        urlExamBeganRe = requests.post(urlExambegan, json=InterfaceMultiPara.win_exam_begin(i, session))
        print("测验开始接口调用返回值为")
        ...
        for question in questions:
            ...
            if quesType == 0:
                ...
                urlCommitRe = requests.post(urlCommit,
                                            json=InterfaceMultiPara.win_step_commit1(session, i, questionId, quesType,
                ...
                print("开始提交答案，题号为：" + str(questionId))
                urlCommitRe = requests.post(urlCommit,
                                            json=InterfaceMultiPara.win_step_commit2(session, i, questionId, quesType))
                print("提交答案的返回")
                print(urlCommitRe.text)
        print("开始请求测验结束接口")
        urlExamFinishRe = requests.post(urlfinish, json=InterfaceMultiPara.win_exam_begin(i, session))
        print("测验结束接口返回")
        print(urlExamFinishRe.text)
    print("end")

for i in range(1,120):
    runAnswer(str(17006000000+i))
